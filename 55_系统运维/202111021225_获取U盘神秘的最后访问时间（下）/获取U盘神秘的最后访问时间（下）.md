...
此时简睿正讲到注册表中因限于 `SYSTEM` 权限而止步于访问 `Properties` 子项。
...
蔡学飞边记边点头，“原来如此，那我想要查找指定的U盘，只要按前面说的方法，定位相应的 `GUID` 后找到十六进制的时间数据，然后再将其转换就可以了吧？”
...
“需要注意的是，`PsExec` 作为远程工具，它有一个小缺点，那就是必须要保证程序所在系统开放了默认共享（`C$` 、`Admin$` ），否则会导致程序启动失败。”
...
“我花了几个晚上的时间，自己动手用 `VB` 制作了一款小工具 `XJUsbViewer` ，USB时间记录查看器。”
...
简睿补充道：“我利用了第三方的 `NirCmd` 程序，在程序启动时自动切换以 `SYSTEM` 权限运行。

然后再点击界面上的 `查看USB时间记录` 按钮，系统中的所有USB时间记录就都会显示出来了。
