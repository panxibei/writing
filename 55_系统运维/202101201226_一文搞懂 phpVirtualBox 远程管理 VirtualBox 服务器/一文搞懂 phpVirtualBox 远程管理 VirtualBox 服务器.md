...
实际上这是由于使用的 `phpVirtualBox` 版本所致，**建议用本文中的版本（免费下载）**。

官方主分支给出的是 `5.2`  版本，由于某些不可描述的原因，原维护者没有在原版本上继续升级维护，而其他网友加入进来后，目前只有最新开发版本才能适用于 `6.1` 。

这个坑我已经帮你们踩好了，即使是通过 `SDK` 中的文件替换也不好使，请不要轻易尝试。
...
我查遍了整个互联网，只有前面坑一说的那种做法，但并没有修复我的问题。

难道我查了个假的互联网？

这个问题的确挺诡异的，我遇到的实际情况是，在自己电脑上怎么试都不行，而换了台电脑，同样的环境却没有问题。

最后才发现，可能是我的 `PHP` 环境有问题，从 `7.3` 切换成 `7.4` 瞬间就好了！

当然，这并不是 `PHP` 版本的问题，因为我在其他电脑上同样是用的 `7.3` ，所以应该是我自己电脑本身的问题吧。
...
如果只是管理一台服务，那可能就没必要大费周章搞 `phpVirtualBox` 。

编辑 `config.php` 文件，大概在39行处，将注释取消，修改成相应的服务器连接信息。
...
重新登录 `phpVirtualBox` 页面，在左上角就可以通过服务器名称查看和切换目标服务器了。
...
`phpVirtualBox` 默认会将用户名和密码保存在多台中的某一台服务上以便能正常访问它们。

默认情况下会保存在 `$server` 数组列表中的指向的第一台服务器中，不过也可以通过参数 `'authMaster' => true` 强制指定服务器。
...
var $servers = array(
        ...
        array(
                ...
                'password' => 'pass2',
                'location' => 'http://192.168.1.2:18083/'
            	'authMaster' => true	// 指定保存用户名和密码的服务器
        ),
);
```
...
这个时候如果你刷新页面想重新加载登录，那么你会发现无论如何也无法回到过去了，它一直提示这个错误。

不要紧张，想回到过去其实也很简单，你仔细认真地看看这个错误提示。

发现什么鬼没有？

只要你点击那个 `Server List` 后面的有效在线服务器的名字就可以切换正常了。

我是在重启服务、修改配置等等众多坑都被踩遍了之后才发现的！
