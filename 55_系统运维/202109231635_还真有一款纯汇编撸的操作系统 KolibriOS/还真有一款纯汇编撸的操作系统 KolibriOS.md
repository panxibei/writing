...
这里的程序都是点击后秒开，官方也说了，他们就是想要消灭鼠标沙漏，也就是所谓的等待时间，真是太棒了！

此外，窗口也没有最大化功能，标题栏上按下鼠标右键，窗体就会收缩只剩下标题栏，这个有点像某些 Linux 系统。



##### 文件资源管理器

`KolibriOS` 的大神们创造了三个文件资源管理器，要不怎么说他们是大神呢！

图06



从抓图中我们能看到的确有三个文件资源管理器，分别是 `KFM2` 、 `EOLITE` 和 `KFAR` 。

为啥大神们要搞出来三个呢，炫技吗？

当然不是了，你看看这三个不同的管理器，单从界面上就能看出来，它们的风格完全不一样啊。

`KFM2` 是 `KolibriOS` 自家的，而 `EOLITE` 一看就像是仿 Windows 的风格，还有 `KFAR` 妥妥的 DOS 复古风格。
...
`KolibriOS` 是用汇编语言 `FASM` 编写的，正是这一点使得它的性能非常高、运行效率等非常出色。

但是如果我们仔细想想，要是让我们一行一行地写汇编代码，就算是累吐了血也不可能完成的啊！

那理所当然大神们也不想为了一个系统而累到吐血，因此他们在搞系统的同时也开发了一个可以用 `FASM` 写代码的编辑器。
...
Linux 下的我就不再赘述和演示了，都差不多，而具体的用法，就留给喜欢玩汇编的小伙伴们思考动手吧，多年荒废汇编的我就不在这儿班门弄斧了。



##### 网络

一个不能联网的系统是个没有灵魂的系统，`KolibriOS` 当然不是了。
...
设置好网络，我总要 `Ping` 一下吧，可是当你打开桌面右上角的那个 `SHELL` 后，你就会发现，这个窗口里根本就没有 `Ping` 命令。
...
怎么可能嘛，其实只要点击开始菜单，顺着网络一项就可以找到 `Ping` 命令了。

直接输入 `IP` 地址就可以开始测试网络了。
...
大神们在写程序的时候总会疲惫，稍事休息可以理解，但 `KolibriOS` 中却放上了三十多款游戏。
