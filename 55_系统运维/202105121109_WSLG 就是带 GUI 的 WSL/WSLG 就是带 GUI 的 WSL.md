...
英文：wslg-is-short-for-windows-subsystem-for-linux-gui

关键字：wsl,wls2,wlsg,windows,subsystem,linux,gui,qq,preview



前文书我们囫囵说了说安装 Windows 10 预览版的事儿。

为啥要安装预览版呢，其实主要是为了玩一玩 `WSLg` ，也就是支持 `GUI` 图形软件的 `Linux` 子系统。

要说这玩意有啥好的，嘿嘿，不懂了吧，这可是高级货哦！
...
好了，看过前面的文章，Windows 10 预览版我们已经可以搞定了。
...
当前版本 `21376.1` ，好像比 `21362` 大那么一点点哈，我数学不好，小伙伴们帮我看看。

OK，既然大那么一点点，那么我们进入下一步。
...
有很多很多，我只找了两个常见的程序，一个是 `gedit` ，一个是 `linux qq` 。
...
虽然安装完成了，但可能还有一些库文件缺失，还需要我们自己再安装一下。
...
不过嘛如果你是在已经安装有 `WSL` 的系统上更新为预览版的话，那么其实只要两条命令就可以了。
...
2、如需手动安装 `WSL` 所需组件的话，可以用以下命令，这些是不需要联网的。
