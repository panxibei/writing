...
这里需要简单地说到 `PGP` 的概念，`Pretty Good Privacy` 中文为 “优良保密协议”，是一套用于消息加密、验证的应用程序。

而 `OpenPGP` 是开源 `PGP` 的统一标准协议，`GPG` ( `GnuPG`) 就是符合 `OpenPGP` 标准的开源加密软件。

很明显上面的数字签名的文件内容就是基于 `OpenPGP` 的。

那么我们就很容易明白，我们需要用到 `GPG` 程序来校验文件。



一会儿 `PGP` 、一会儿 `GPG` ，是不是有点头晕啊？
...
其实只要安装 `GPG` 程序，`Windows` 也照样能玩得起来。



### 安装 `Gpg4win`

开源软件从来都不会忘记 `Windows` ，在 `Windows` 平台上也提供了名为 `Gpg4win` 的加密程序。

> 官网地址：https://gpg4win.org



我们来到下载页面，如果你此时身无分文，那么也没关系，点击那个 `$0` ，就可以完美跳过捐赠直接下载了。
...
但是有时也会出现如下，说签名文件过期（ `This key has expired!` ）。

那么我们可以用一个取巧的办法，就是将系统当前时间手动修改为签名文件有效期之前，然后再一次执行校验命令。
...
命令行操作基本上够用了，不过毕竟在 `Windows` 上我们主要还是要用 `GUI` 程序嘛，这样比较直观易操作，因此我们打开 `Kleopatra` 来看看如何操作校验文件。
...
这也是我一开始懵逼的地方，但我大体知道非对称加密的一些基础知识，因此我就在网上开始寻找密钥服务器。

由于保存公钥是一个极其艰巨的工作，因此能够提供存放并且查询公钥的网络服务少之又少。
...
不过有可能会失败，原因猜测可能是 `KeyServer` 设置有问题，我是将 `Kleopatra` 设置中的密钥服务器修改成 `hkps://peegeepee.com` 后才成功的。

图12



如果你不想修改设置，那么还可以用另一个命令尝试获取公钥证书。
...
实际上 `PGP` 用来校验文件只是它的一个用途罢了，主要它还是用作信息的加解密、签名等等。

比如，当我们打开某个程序文件（比如 `gpg4win-3.1.16.exe` ）的属性窗口，就能看到其中的多了一个数字签名的选项卡。
...
当然了，数字签名只是 `PGP` 的应用之一，但只要有信息的交换，就一定有它的身影。
