...
随着巨硬 `Office 365` 的普及使用，很多职场人的日常工作之一就是将文件上传到 `Sharepoint` 。

上传文件这一很普通的操作，为啥今天我们还要专门拿出来说事呢？

因为看似普通的操作，然而有时会遇到令人头痛的情况。



比如当文件很多，以人力上传肯定是费时费力。

又比如有些文件是系统产生的，需要系统自动上传而非人为。

总之越是重复性的工作，越是需要自动化来替代。

如果你不自动化，那么将来 `AI` 也会推着往向走，逼得你不得不自动化。



我们暂且先不讨论关于 `AI` 的那些有点残酷未来，我们就在这儿简单讨论一下，就目前情况我们怎么把眼前这个上传问题给它自动化处理一下。

解决的方法肯定很多，不过有人尝试过用 `VB6` 吗？

其实有小伙伴曾提出使用 `VBA` ，也是可以实现的。



上传文件，肯定要用到 `http` 方面的知识啊，用 `VB6` 来实现 `http` 请求，听上去好像开玩笑一样哈！
