...
创建播客
 ...
 多亏了互联网，每个有影响力的人都在制作播客。现在，任何拥有一点录音设备和互联网连接的人都可以创建音频广播与世界分享。你可以讲故事、报道新闻和分享操作方法。你的播客可以只是你一个人说话，你采访另一人，你和一个联合主持人，或者你和一群联合主持人及嘉宾。你可以添加音乐曲目或特效。使用Audacity制作播客既简单又有趣。一旦你创作出了你的杰作，你需要一个网站来托管它并让充满期待的听众访问它。互联网上充满了免费和廉价的播客托管服务。如果你有足够的带宽并且想成为自己的服务器管理员，你可以依靠众多免费的开源流媒体服务器软件自己进行托管。播客这个词过去指可下载的、为iPod编码的MP3录音。苹果曾经短暂而激烈地试图拥有“pod”这个词，并派律师去追究了一些敢于使用这个词的小公司，比如TightPod（笔记本保护套）、Profit Pod（售货机数据收集器）、Spodradio（一家德国在线广播公司）和Podcast Ready（另一家在线广播公司）。幸运的是，看起来侵权律师已经转而去做别的事情了，现在再次使用“pod”是安全的。所以，这正是我们要做的：豌豆包、鲸鱼群、人群。打开太空舱门，Hal。 



 Audacity 是制作播客的绝佳工具。你可以使用数字录音机，然后将录音导入 Audacity 进行编辑，或者你可以直接在电脑上录音。将 USB 麦克风或耳机连接到 netbook，就可以组成一个轻量便携的录音室。或者，你可以尽情使用整个配备昂贵设备的录音棚；完全取决于你。到目前为止，在本书中我一直在讲高质量音频和不使用有损压缩音频文件格式的内容。对于播客，我们走的是另一种方式：更小的文件大小和有损压缩格式。你仍然希望首先创建一个高质量母带，但不必像对待终极发烧音乐录音一样对待你的播客，因为它将被剪辑以适合互联网流媒体和快速下载。你可能想先回顾第 1 章和第 2 章，了解使用 Audacity 的基础以及如何设置你的录音设备。 
...
2. 在 Audacity 中编辑：裁剪、修复、标准化——通常的清理和修正操作。

3. 导出为可播放的压缩格式，如 Ogg Vorbis 或 MP3。

4. 上传到一个网站，让你的粉丝下载并享受你的播客。
   让我们来看看制作一个好播客的细节。

   

> 注意：在定义轨道时，音频术语有点混乱。轨道可以是单声道轨道、立体声轨道，甚至是声道。 在 Audacity 中，将轨道视为任何可编辑单元是有帮助的，这包括任何单声道轨道或立体声轨道。你所做的任何编辑，如删除、添加、特效等，都将同等应用于 Audacity 立体声轨道的左声道和右声道。 



##### 制作一个简单的语音播客

最简单的播客就是你自己讲话。虽然听起来很简单，但对着麦克风讲话还是需要一些练习。你可以练习对着麦克风讲话，同时避免产生不必要的噪音，例如嘴巴发出的咂嘴声、噼啪声、大声呼吸、假牙的咔嗒声或其他令人不快的声音。注意像“呃、嗯、你知道”这样的口头语。练习掌握嘴巴与麦克风之间的最佳距离，并将麦克风设置到你可以舒适地站立或坐着的位置。使用麦克风支架比手持更好，如果你使用领夹麦克风，请夹在衣物不会摩擦到的地方。 

 我更喜欢高质量的USB耳机。说话时要面向麦克风就好像你面前有观众，而不是直接对着麦克风说；这将帮助你提升声音的活力并以一种减少不良副作用的方式发声。防风罩或泡沫防风套是必需的。你应该说多快？这取决于你的个性和材料类型。通常每分钟120个单词被认为是一个较好的速度，尽管对大多数英语母语者来说会感觉较慢。这并不一定意味着每秒只能说两个单词，而是意味着放松并使用精心安排的停顿，让听众有机会吸收你所说的内容。我使用Plantronics立体声USB耳机进行语音录音（图7-1）。它舒适且易于使用，音质也很好。USB录音接口具有自己的模数转换器，所以你不需要声卡。它们还消除了拾取计算机内部噪音的风险，而这是内部声卡有时会遇到的问题。 
...
图7-1
 ...
 要在没有稿子或排练的情况下说得好需要大量练习，所以如果你需要这些辅助工具，不要觉得难为情。尽量做一个尽可能干净的录音比事后去清理充满问题的录音要好，而且你的听众会更欣赏清晰、集中精力的展示。 



##### Audacity 设置

首先，通过 文件 > 另存项目 为 新建一个 Audacity 项目。然后，在 编辑 > 首选项 > 设备 对话框中设置你的录音和播放设备，并将通道设置为你一次录音所需的数量。如果只是你使用单声道麦克风，则设置为一个通道。如果你使用的是立体声麦克风或耳机，则为两个通道。注意廉价的板载声卡芯片，例如笔记本电脑中使用的那些——麦克风输入通常是单声道，即使它们声称是立体声。

在 编辑 > 首选项 > 质量 对话框中，将默认采样率设置为 44,100，并将默认样本格式（实际上是比特深度；Audacity 有一些术语上的特殊用法）设置为 32 位浮点。（有关比特深度和采样率的更多信息，请参见第 1 章。）如果你愿意，可以以更高的音频分辨率录音，但对于简单的语音播客来说，这些额外的位数不会有任何区别。实际上，你也可以使用 16/44.1。这可以节省磁盘空间，音质不错，并且能很好地导出到低质量格式，如 MP3 和 Ogg Vorbis。 



 现在前往 编辑 > 首选项 > 录音 对话框，确保“叠加：在录制新轨时播放其他轨”和“软件监听：录制或监控新轨时监听”未被勾选。退出首选项菜单，点击输入电平表开始监听并设置你的录音电平（见图7-2）。根据你的声音变化情况，这可能需要一些反复试验。将录音峰值设定在 −9 或 −12 dB，以给自己一个良好的安全余量，你仍然会有非常好的信噪比。切勿超过零，这会导致削波，从而产生失真。蓝色线表示本次会话的最高峰值，红色线表示过去三秒的最高峰值。深红色条表示峰值电平，浅红色条显示平均振幅。 
...
图7-2
 ...
 大多数 USB 耳机都带有 Windows 软件，可以让你控制输入和输出音量。Linux 用户可能需要依靠 ALSA（参见第 13 章）来设置音量水平。Audacity 混音器工具栏（图 7-3: Audacity 混音器工具栏）在音频接口驱动支持音量控制的情况下可以使用。很多驱动是不支持的。这就是为什么我喜欢带有普通、老式旋钮可调节的录音接口的原因。现在，到了你期待的时刻——点击录音按钮开始说话！图 7-4 显示了正在进行的语音录制。点击暂停可以停止录制，点击录音可以将新内容附加到现有轨道。如果你点击停止后再点击录音，将会创建一个新轨道。如果你不小心点击了停止，可使用 SHIFT-录音从上次停止的地方继续录制。 
...
图 7-3
 ...
 清理你的录音 一旦你记录下了你的深度思考，你可以回过头来清理你的录音。首先，删除任何你不想保留的内容；在第一次处理时只需做一个粗略的剪辑，并多留一点空间。你总是可以再删除更多，但很难轻易恢复。如果你有多个音轨，要注意保持它们同步（虽然如果它们不同步，有简单的方法可以让它们重新同步）。然后选择视图 > 显示削波，以红色突出显示任何削波片段。如果你发现有削波片段，有多种方法可以修复它们。如果是非常短的削波，不到四分之一秒的长度，可以放大直到你能精确选择削波部分并删除它。当它只有一小部分秒时，没有人会注意到。 

图7-4



 另一种方法是在第二个音轨或 Audacity 窗口中以正确的录音电平重新录制被削波的片段，然后将其复制到被削波的片段上。手术式地替换一个短片段并不容易，因此你可能需要选择一个更长的部分进行替换。寻找自然的断点，这样你就可以平滑地拼接新的部分，如图 7-5 所示。图 7-5 中的选区大约有四秒长，尽管被削波的部分（由六秒到六点五秒之间的三个垂直峰值表示）持续时间不足半秒。 

图7-5



 Nowhere 是我喜欢的一个很酷的技巧，因为它帮我跟踪我正在编辑的位置。如果你经常使用 Audacity，你会知道鼠标滑动一下或者不小心按到热键就可能会丢失编辑位置，如果是长音轨，回到之前的位置会很麻烦。所以，不要删除你的选区，而是使用 编辑 > 分割删除。这会用一个空隙替换选区，如图 7-6 所示。这也会保持音轨的长度不变，如果你有两条或更多的音轨需要保持同步，这可能会很有用。 

图7-6



 现在你可以继续进行重新录音。不要担心录音的音量与原项目完全一致，因为这可以通过效果 > 放大轻松调整。当录音满意后，没有任何打嗝、碰撞或咳嗽，将其复制并粘贴到空白处（图 7-7）。边缘可能不完全匹配，但我们可以修复它。 

图7-7



 有多种方法可以处理新片段两侧的空白部分。最简单的方法是删除它们。如果你不想缩短音轨，可以使用生成 > 静音来填补空隙。另一种方法是使用时间移动工具将所有内容一起移动。当你看到对齐指南出现时，边缘会完全匹配，这是一条黄色的垂直线，当边界完美对齐时会出现。点击以标记边界连接的位置，然后放大到可以使用修复效果平滑处理补丁边缘的程度。此效果操作的最大样本数为128个采样，因此需要放大到可以选择如此小的范围。缩放工具会以你点击的位置为中心。然后点击效果 > 修复，该效果通过插值使接缝不明显。另一个有趣的 Audacity 技巧是使用效果 > 变更速度来延长一个补丁而不改变音高。你可以通过在选择工具栏中选择“长度”单选按钮来精确测量所需长度，然后从补丁的起点选择到空隙的终点。当你正好在边界上时，黄色对齐指南会出现。图7-8中的示例需要长60秒。然后只选择你的补丁，点击效果 > 变更速度，输入新的长度，如图7-9所示。点击预览按钮，如果听起来没有太奇怪，点击确定，即完成操作。 
...
图7-9
 ...
 降低过高的峰值 图7-9：使用效果 > 改变速度来拉伸一个片段 你还应该查看波形中是否有任何不成比例的高峰（图7-10，在2.7到3秒处），即使它们没有被削波，也要评估是否需要降低它们。在这个例子中，我由于制作播客而过于兴奋，声音说得太大，所以我使用了效果 > 放大来降低振幅。是的，它写着“放大”，但如果你给它一个负值（例如−3或−6 dB），它将会衰减。你可以通过反复试验来确定需要降低多少音量，或者你可以使用波形(db)视图来了解你需要减少多少。通过轨道菜单将波形显示更改为波形(db)（图7-11），然后扩展轨道宽度（用鼠标抓住边缘并拉动），直到看到左侧的分贝刻度（图7-12）。在这个例子中，看起来响亮的部分可能高出了多达12分贝，因此这给了我尝试−12的起点。记住，Audacity几乎提供无限次撤销（编辑 > 撤销或 CTRL-Z），所以使用反复试验是安全的。 
...
图7-10
 ...
 轨道面板中的分贝刻度可以在“编辑 > 首选项 > 界面”对话框中调整，范围从 0 到 −36 到 0 到 −145。这样可以根据你的需要调整显示。我通常保持在 −60，因为这样我可以获得足够的细节，同时不会让刻度过大。 
...
###### 淡入淡出
 ...
 图 7-12：这个选择可能会高达 12 分贝。控制淡入和淡出有两种方法。简单的方法是选择你想要淡入或淡出的轨道段，然后点击效果 > 淡入或淡出。你要做的就是控制淡入或淡出的长度。要进行更精细的控制，可以使用包络工具。点击波形以创建控制节点，然后拖动这些节点来使轨道的不同部分变得更响或更静。将节点拖到轨道边界之外可以删除它们。 



 归一化 当你的音轨都修剪并清理完毕后，最后一步是应用效果 > 归一化。在归一化对话框中，确保两个选项都被勾选（“移除任何直流偏移”和“归一化最大振幅”）。直流偏移指的是平均振幅；如果它不为零，那么归一化将无法正确应用，因为振幅水平会失衡，甚至可能产生一些失真。图7-13展示了直流偏移的示例，修正前后对比。在上方的音轨中，你可以看到整个波形没有完全以零为中心，而在下方的音轨中则是如此。 
...
图7-13：归一化前（上图）和归一化后（下图）现在录音都清理好了，可以导出为压缩有损格式，比如Ogg Vorbis或MP3（MPEG-1音频层3，不是MPEG-3）。（请参阅第13章和第14章，了解如何在Linux和Windows上启用MP3支持。）MP3 是通用便携数字音频格式：所有设备都支持 MP3。正如我们在第一章讨论的，Ogg Vor bis 和 MP3 之间存在差异。一些听众声称Ogg Vorbis的音质优于MP3。在低码率下，这两者听起来都不太好;相反，它们听起来平淡无奇，缺乏个性和深度。在较高码率下，两者都能听到AC，不过我觉得Ogg的声音更准确、更自然。MP3听起来有点太亮、太刺耳。Ogg Vorbis 支持多声道环绕声，而 MP3 支持一声道或两声道。MP3 受制于多个不同国家的专利持有人，最后一项专利将于2017年到期。这是否意味着如果你以MP3格式分发作品，攻击律师会来找你？我不知道。看起来很多艺术家会把作品分发成MP3，不付授权费，也不会惹麻烦。请咨询专门处理这些问题的律师。即使你的听众都拥有巨型宽带，保持节俭态度也是个好习惯。而且它们并非所有烟斗都粗粗;美国拨号上网用户估计占25%到45%。别忘了，Inter Net是国际的，所以你的潜在受众包括依赖拨号和低速无线的国家。即使是宽带用户，网络控制和排队延迟流媒体，所以教训是，低码率媒体传输仍然更可靠。一如既往，权衡是必要的。如果你的播客主持人允许，你可以为听众提供不同质量的收听选项，包括流媒体和可下载文件。 



###### Ogg Vorbis 质量设置

在 Audacity 中导出 Ogg 很简单。你可能读过一些教程建议使用可变比特率（VBR）而不是固定比特率（CBR）。虽然这是个好建议，但其实没有必要，因为 Ogg Vorbis 不支持 CBR。选择 文件 > 导出 > 文件类型：Ogg Vorbis 文件，然后点击选项按钮（图 7-14）。导出到 Ogg Vorbis 很简单：选择一个数字。0 是最低质量和最小文件大小，10 是最高质量和最大文件大小。 

图7-14



可变比特率意味着简单的段落会有较低的比特率，而更复杂的段落会有较高的比特率。这比固定比特率提供更好的音质。你无法精确计算文件大小，但发明 Ogg Vorbis 的那些好心人本来就不太认为用比特率来衡量音质有什么意义。为了提供一个比较的基础，质量设置为 3 时，平均比特率约为 112Kbps，而 Ogg 爱好者声称它听起来比 128Kbps 的 MP3 更好。Ogg Vorbis 使用有损通道耦合，最高可达 5 级。左右声道之间的冗余会被合并以节省空间。这可能会影响立体声成像，所以如果这让你在意，请使用 6 或更高的质量设置，因为在这个级别会关闭通道耦合。我使用 6。我的耳朵无法分辨 7 及以上的差别。当然，你的耳朵可能会有不同的感受。对于简单的语音播客，你可以一路降到 0，但它仍然听起来可以接受。 



###### MP3 质量设置

在 Linux 和 Windows 上，你可能需要单独安装 MP3 编码器。使用 LAME MP3 编码器，因为它质量高、免费且开源。Linux 用户可以从常用软件仓库安装，Windows 用户应按照 Audacity 下载页面的说明操作 (http://audacity.sourceforge.net/help/faq?s=install&item=lame-mp3)。 

> 注意 142 Windows 用户应访问 OSSwin 项目（http://osswin.sourceforge.net/）以查找数百个优秀 Windows 开源应用程序的链接。 



我不知道音频爱好者是喜欢为了争论而争论，还是对一切事情都充满强烈的热情，因为 MP3 质量设置是争议的雷区。只遵循这个简单的规则，你就没问题了：我，你的作者，是对的（除非你自己的耳朵告诉你不同）。在 Audacity 中，你有四个类别可以设置 MP3 比特率：预设、可变、平均和恒定。预设提供了由 LAME 开发者推荐的四种不同的预设组合：中等、标准、极端和疯狂。标准适合音乐，但如果是简单的语音广播，你可以用较低的设置。中等、标准和极端都使用可变比特率（VBR）。可变比特率意味着比特率会根据声音的复杂性变化，从而获得更好的音频质量。在过去，可变比特率会让 MP3 播放器头疼，因为它们是为恒定比特率（CBR）设计的。但那是很久以前的事了，如今可变比特率不应再造成任何问题。可变比特率总是在一个范围内定义，例如标准预设的 170Kbps 到 210Kbps。疯狂使用最大恒定比特率 320Kbps。根据我的经验，标准设置（170Kbps 到 210Kbps）是 MP3 所能达到的最佳效果，超过这个范围并不会产生明显差异。可变比特率有十个质量设置。质量设置 5 对应 110Kbps 到 150Kbps，这对于简单的语音广播已经足够。平均比特率在我看来不值得费心，它是可变比特率和恒定比特率之间的妥协。你设置一个恒定比特率，但由于它是平均值，允许稍微波动。因此质量不如可变比特率好，比恒定比特率稍好。恒定比特率在通过互联网流式传输音频时表现更好，而且文件大小可预测。如果你想使用恒定比特率，Audacity 提供了完整的选择范围。设置可变速度：标准。标准提供更好的质量，并且编码时间略长。图 7-15 和图 7-16 显示了 MP3 导出菜单及各种质量设置。 



 保持声道模式：立体声，因为只有在对存储空间极度紧张的情况下，你才会考虑使用联合立体声。如果你对存储空间如此绝望，以至于考虑将 MP3 文件编码到 112Kbps 或更低比特率，那就可以尝试一下。联合立体声这个术语起得不好，并不能准确描述它的作用。一个普遍的误解是，它会将两个立体声声道合并，从而破坏立体声分离。就像音频工程中的事情一样，情况比这复杂得多。这里有一个新的音频术语给你：人类心理声学。这个词组是不是很棒？不，它不是指像拿着大刀的疯子一样的精神病患者，抱歉。它是指以考虑人耳如何感知声音的方式来设计音频，而不是严格遵循数学模型，因为数学模型并不符合人类真实的听觉工作原理。这相当复杂，如果你不喜欢联合立体声的声音，也很容易重新处理。 
...
什么是联合立体声？在 MPEG-1 音频第 3 层标准中，它指的是使用多种技术之一，通过合并左右声道的冗余来在保持可接受音质的同时降低比特率。这些“冗余”是人耳感知为方向性较低的频率，也就是高频。它应该能改善低比特率录音的音质，因为在牺牲立体声成像的情况下保留了更多的音频数据。 



```
MP3、编码器和其他音频格式
LAME代表“LAME不是一个MP3编码器”。在最初的时候，它并不是一个编码器；它是一组针对原始ISO/IEC演示源代码的补丁，因此（严格来说）技术上它并不是一个编码器。随着时间的推移，LAME的开发者编写了全新的代码，直到LAME不再需要ISO源代码，现在它显然已经是一个编码器。由于围绕MP3（MPEG-1音频层3）格式存在令人困惑的专利问题，因此开发了几种替代格式：Vorbis（开源）、高级音频编码（AAC）和Windows媒体音频（WMA）。AAC和WMA也有其自身的问题，稍后将讨论。没有任何MP3专利持有人曾经追究开源编码器的开发者或为了个人私用而使用MP3的个人。
```



 其他有损格式 还有其他有损压缩的音频格式可以生成较小的文件，如 AAC、WMA 和 RealPlayer。Audacity 在 Linux 和 Windows 上通过 FFmpeg 库支持这些格式，并提供了一些很好的导出预设。你也可以使用你选择的外部编码器，可以通过导出对话框中的“另存为类型”下拉菜单访问（图 7-17）。 

图7-17



> 注意 图 7-17：使用“外部程序”选项可以使用任何已安装的音频编码器导出。请参阅第 13 和第 14 章了解如何安装 FFmpeg 和 MP3。 



 AAC直到苹果选择它作为iTunes商店和所有音频设备的默认格式之前，并不特别流行。WMA是Windows Media格式，而RealPlayer则……嗯，就是RealPlayer。AAC、WMA和RealPlayer都是封闭的专有格式，带有各种令人不悦的特性，如数字版权管理(DRM)、跟踪软件和专利束缚。 



###### 导出您的播客

一旦您决定了要将播客导出为哪种格式或多种格式，选择 文件 > 导出 来进行实际导出。您可以从同一个 Audacity 项目中进行各种导出。请注意，您可以通过选择音轨的一部分，然后选择 文件 > 导出所选内容 来导出音轨的部分内容。 
...
说明你不想成为一个孤独的自说自话的隐士，而是希望有一个联合主持人或希望采访另一个人。最简单也是最好的方法是每个人都有自己的单声道麦克风，并连接到单独的录音通道。这样可以更容易进行编辑和修复。请对你的听众友善，尽量让两个人的音量接近相同。让你的耳朵作为指导，而不是仅凭波形，因为即使波形显示两个人的音量相等，有些人听起来还是会更大声。（又是那个心理声学的小技巧。）如果你使用带有真实立体声麦克风接口的上网本或笔记本电脑，你可以使用分配器来插入两支单声道麦克风。 



######  将双轨录音编辑为单个立体声轨

当你录制两个通道时，无论你的设置如何，Audacity都会创建一个单独的立体声轨。你可以使用两个单独的单声道麦克风或一个立体声麦克风，默认情况是单个立体声轨。图7-18显示了它的外观。（注意波形显示每个人都礼貌地轮流发言。） 

图7-18



 您可以使用很酷的新混音台调整左右声道的相对音量，该混音台首次出现在 Audacity 1.3.8（图 7-19）。使用混音台来调整轨道的增益或音量级别，并使用声像滑块调整左右声道的相对音量。这些调整不会更改项目文件；您将在 Audacity 播放和最终导出中听到更改，但 Audacity 项目文件不会被修改。混音台控件是全或无的；它们适用于整个轨道，不能只应用于轨道的某些部分。



这相当简单，如果您想要更多控制，可以将录音拆分成两个单声道轨道。 
...
将立体声轨道分成两个单声道轨道可以让你获得更多控制。点击轨道菜单中的下拉箭头，然后选择“分割立体声轨道”（图7-20）。现在你有两个独立的轨道，可以对每个轨道单独进行所需的更改和修复。 

图7-20



你应该给每个音轨命名（也可以使用音轨菜单），这样你就总能知道它们是什么。 



> 注意 这是一个关于混音台的酷技巧：如果你在曲目名称中使用某些词，例如 guitar、vocals、piano，甚至一些缩写，图标将会与名称匹配。请参阅第189页的“自定义混音台”以查看关键词的完整列表。 



 在将你的立体声轨道拆分为两个单声道轨道后，分别编辑每个轨道，进行常规修正，并尽可能匹配两个扬声器的音量。默认情况下，当你按下播放时，Audacity 会同时播放所有轨道。在混音板上查找静音（Mute）和独奏（Solo）按钮，以控制你想听的音轨。独奏会自动静音除了你想听的那个轨道以外的所有轨道；静音会一次性静音一个轨道。如果两个人同时说话，你可以通过添加静音将他们分离。例如，如果有五秒钟他们同时说话的重叠部分，可以在一个轨道上添加五秒静音，将它们分开。将光标定位在你希望静音开始的位置，点击生成 > 静音，然后输入你想要的静音秒数。你可能需要在两个轨道上多次进行此操作以保持它们对齐，特别是在进行更改之后。当你完成所有清理并准备导出时，通过使用上方轨道菜单并点击“创建立体声轨道”来重新合并你的轨道。 



##### 添加背景音乐轨道

既然你已经擅长制作多轨播客，添加音乐轨道就像轻而易举。就像制作单人播客时一样，你的音轨即使是单声道录音，也应为双声道立体声轨道。这样，听众就能在两个声道中听到音乐。你可以通过复制单声道轨道，将副本粘贴在原始轨道下方，然后在上方轨道的轨道菜单中选择“制作立体声轨道”来将单声道轨道转换为双声道轨道。

你可以通过打开“文件 > 导入 > 音频”对话框，或者从另一个 Audacity 项目复制粘贴来将音乐文件添加到你的播客项目中。你可以使用增益滑块调整音乐音量。记住，增益滑块不会改变项目文件，因此这是在不对项目进行大量更改而可能影响未来编辑的情况下快速调整混音的方法。另一种选择是使用包络工具（Envelope tool）将其从宜人的背景音量调节到激动人心的高潮。包络工具会改变项目文件。

还有一种更酷的方法来管理背景音乐轨道的音量，那就是使用“效果 > 自动降音（Auto Duck）”。这对于带有背景音乐的录音非常完美，因为当前景轨道出现时，它会自动降低音量，当前景轨道结束时，它会自动升高音量。在图 7-21 中，我们看到设置：背景音乐轨道在上方，下方轨道为人声轨道。当人声开始时，我希望音乐音量降低，当人声结束时，音乐应增大。轨道必须按此顺序排列：背景轨道在上，前景或控制轨道在下。 

图7-21



 选择背景音轨，然后点击效果 > 自动降音。你会看到一个控制面板，如图7-22所示，其中显示了我喜欢使用的设置。这些设置会创建快速淡出和渐进淡入。背景音轨会快速移开，让位给人声轨道，然后以优雅的方式慢慢滑回，同时与人声轨道有一点重叠。 
...
外部淡出长度和外部淡入长度的数值总和不能超过最大暂停。



外部淡出长度设置决定了在控制轨道上的人声回归之前，背景曲目将衰减的速度。

两条垂直线之外的任何内容都发生在控制轨道低于阈值时。

内部淡出长度设置决定了与控制轨道的重叠程度。



外部淡入长度设置在控制轨道上的人声停止时背景曲目回淡入的速度，而内部淡入长度设置控制重叠程度。

在人声上轻轻上升半秒的音乐并不多；我认为它可以作为一个不错的过渡。

阈值设置控制触发自动鸭音效的声音水平。



当你完成后，需要将轨道混合并渲染成一个单声道轨道。

如果按CTRL-SHIFT-M，这会将你的项目混合为一个新的立体声轨道，位于原始轨道下面。

这是我喜欢的混合方式，因为它保留了原始轨道，然后我将新的立体声轨道复制到新的Audacity项目中进行最终调整和导出。

如果你使用轨道 > 混合并渲染，那么你的原始轨道将被新的立体声轨道替换。



有时候合并多个轨道会使音量过高，所以可以撤销混合并渲染（CTRL-Z或编辑 > 撤销），然后使用增益控制将它们调低一些。

然后再次导出。（当你遇到轨道音量异常问题，如声音不平衡时，检查增益滑块。）



现在，你的播客及其背景音乐轨道已经可以导出了。



访问第11章以了解更多关于Audacity的特效，第12章学习修复和清理，第9章了解多轨录音和混音的更多内容。 
...
成为网络明星
 ...
 所以你想要分发自己的录音，甚至可能通过销售获得一些收入。在这个美妙的互联网时代，在线分发既简单又低成本。在线音乐分发成功需要两种不同的技能：一是能够在各种格式下制作好的录音以供下载和流媒体播放的技术能力；二是理解作为专业艺术家的商业运作，这很复杂，涉及客户服务、合同和版权问题等任务。在本章的第一部分，我们将学习如何为不同用途创建不同的音频文件格式，例如低质量的促销MP3、流媒体音频和高质量下载。在第二部分，我们将探讨成为录音艺术家的商业事务，这比成为一个优秀的音乐人和录音高手更为棘手。 



##### 文件格式和音频质量

使用 Audacity，您可以轻松地将音频文件导出为您想要的任何格式。让我们来看看您的选项。常见的可下载格式有 AAC、MP3、Ogg Vorbis、WAV 和 FLAC。 



> 注意
>
> 如果你需要复习如何导出到不同音频文件格式，请查看第1章，并参阅第13章和第14章以了解如何设置和调整系统以进行音频制作。 



 AAC 正变得流行，因为这是苹果 iTunes 商店使用的格式。Audacity 1.3.6 版本引入了通过 FFmpeg 导入、导出和转换非 DRM AAC 文件的支持。这些文件具有 M4A（音频）扩展名。M4P 文件则受 DRM 保护。（不，它的 P 并不是 Pooey，而是 Protected，即受保护的意思。）在 Audacity 中创建 AAC 格式文件很简单：选择 文件 > 导出 > 文件类型保存为：M4A（AAC）文件（FFmpeg），然后点击“选项”按钮。在“选项”对话框中，你可以从 10 到 500 的范围内选择音质设置，其中 500 是最高音质。这个范围对应于公平（Fair）、一般（Okay）、良好（Good）、更好（Better）和最佳（Best）。对于更复杂的录音，256 的设置效果不错。对于非常简单的录音，比如单人语音作品，在 50 的设置下听起来也不错（图 8-1）。 

图8-1



 Audacity 支持许多不同的 MP3 选项，你可以在第 7 章中了解所有相关内容。让我们回顾一下最实用的选项。MP3 支持恒定比特率和可变比特率。可变比特率提供更好的音质。你在线分发的站点会有具体要求，而使用 Audacity，你可以轻松地调整你的音乐以满足这些要求。在美国，128Kbps 的恒定比特率 MP3 是最低的通用标准。它可以在每个数字音频播放器上播放，有些分发商还要求恒定比特率 MP3 用于流媒体。在 Audacity 中，点击 文件 > 导出 > MP3 文件 > 选项 > 恒定 > 128Kbits/sec。 

 选项对话框为你提供了从8Kbps到320Kbps的多种固定比特率选项。尽管320Kbps经常被宣传为CD质量，但实际上并非如此。你自己的耳朵可能会有不同的感受，但我听不出相比192Kbps有任何提升。 



> 注意 一些低带宽的运营商在流媒体互联网音频时可能要求比特率低至32Kbps。虽然这听起来可能不太理想，但它扩大了你的潜在听众。在美国，一半的互联网用户仍在使用拨号上网，许多其他国家仍主要依赖低带宽的互联网服务。 



 在 Audacity 中，你的可变比特率 MP3 最佳选项位于 文件 > 导出 > MP3 文件 > 选项 > 可变 对话框中。选择你想要的质量设置，设置可变速度：标准，通道模式：立体声（图 8-2）。此菜单中有 10 个比特率选项，从 45Kbps 到 85Kbps，再到 220Kbps 到 260Kbps。我喜欢 170Kbps 到 210Kbps 作为音乐的合适水平，而 110Kbps 到 150Kbps 适合简单的语音作品。可变速度选项将可变比特率限制在一个设定范围内，这样你既能获得更好的质量，也能获得一定的可预测性。 

图8-2



 如果您的分销商支持，您也应该为不喜欢 MP3 的用户提供 Ogg Vorbis 文件。他们可能更喜欢 Ogg 的高质量，或者他们可能更倾向于使用开源、无专利限制的格式。Ogg Vorbis 可以像 MP3 一样进行流式传输或下载。它在低比特率下提供高质量音频，并支持多声道环绕，而 MP3 只支持一到两个声道。MP3 是一个封闭的、专有的标准，充满了混乱、复杂的专利冲突。我没有听说过任何专利持有人会追究使用开源编码器（如 LAME，Audacity 推荐使用的 MP3 编码器）的人，也没有听说过小规模用户因分发他们的 MP3 格式作品而遇到麻烦。只需注意，这对您的一些粉丝来说是一个问题，并且有可能会成为您面临的问题。



Audacity 支持 Ogg Vorbis，并且选择质量等级非常容易。进入 文件 > 导出 > Ogg Vorbis 文件 > 选项，您会看到一个带有小滑块的菜单，可以从 0 到 10 的 11 个质量设置中选择。数值 3 大致相当于 128Kbps 的 MP3。 

 音乐的数值为5非常不错，但有一个可能的缺点：Ogg Vorbis 在所有不高于5的级别上都使用通道耦合。这意味着两个立体声通道之间的冗余会被合并以节省带宽，因此你可能会注意到立体声成像的损失。通道耦合在6级及以上不再使用。我个人没有感觉到超过7会有任何增益，尽管你的耳朵可能会有不同的感觉。Ogg Vorbis 的支持正在快速增长，许多播放设备都支持它（图8-3）。 
...
图8-3
 ...
 FLAC 是一种出色的无损压缩格式，它能在更小的文件大小下提供 WAV 质量的音频。FLAC 文件通常比 WAV 小大约三分之一。FLAC 非常适合喜欢从电脑播放音乐或拥有基于电脑的音乐服务器的爱好者，并且越来越多的音乐播放器，无论是家用还是便携式，都支持 FLAC。FLAC 不能进行流媒体播放，必须先下载后再播放。FLAC 支持16位和24位的位深，并具有九个压缩等级，从 0 到 8（图 8-4）。默认值是 5，这是一个不错的选择，尽管尝试其他选项也不会有人责怪你。不同的设置表示不同的压缩等级，0 为压缩最少，8 为压缩最多。FLAC 是无损的，所以音质没有差异，只是文件压缩后的大小不同。值为 8 的压缩比编码需要的时间比值为 5 的长好几倍，但额外的好处不大。对最大压缩的 FLAC 文件进行解码并不比轻度压缩的文件耗时更长，因此压缩程度不会影响播放。 

图8-4



 WAV 是高质量无损音频的标准。文件体积大，无法进行流式传输，但对于希望获得可轻松转换为其他格式（例如 CD 和低质量便携格式）的高质量文件的粉丝来说，这些文件非常棒。当然，它们也适合用来直接欣赏。依次点击 文件 > 导出 > 其他无压缩文件 > 选项。在“无压缩导出设置”对话框中（图 8-5），选择 头信息：WAV (Microsoft)，然后选择你的编码。可选择 有符号 16 位 PCM 或 有符号 24 位 PCM。16 位是 CD 质量，24 位是 DVD 和录音室母带质量。仅当你的原始录音是 24 位或更高时，才选择 24 位。 

图8-5



 有符号数表示正整数，这是我们在第一章中学习的内容。你需要有符号的16位或24位整数，因为这些是在Audacity之外可播放的格式。32位浮点数是一种中间编辑格式，由Audacity和其他音频编辑器内部使用。 



##### 唱片艺术家的业务

你正认真考虑分发自己的录音。你已经听过所有关于艺术家如何被娱乐行业剥削的可怕故事，并且已经记住了Courtney Love在2000年5月16日数字好莱坞在线娱乐大会上的精彩演讲“Courtney Love 做数学”（http://salon.com/technology/feature/2000/06/14/love/） 



 今天我想谈谈盗版和音乐。什么是盗版？盗版是指在没有任何支付意图的情况下窃取艺术家的作品。我不是在说 Napster 类的软件。我说的是主要唱片公司的录音合同。 

 弗兰克·扎帕和贾尼斯·伊恩也以类似的方式写过文章或进行过讲座，你应该能够在网上找到这些资源。或者，也许你并不担心与大型唱片公司签约可能带来的不愉快经历，但你希望保留对自己命运和作品的控制权。你希望发布按照你规格制作、并针对不同用途量身定制的录音，而不是为了最低的标准降低一切。你希望在自己想要的时间和地点工作，而不是让别人告诉你该做什么。当有人真正想购买你的作品时，你希望尽可能地简化购买流程并且保留收益。那么，你该怎么做呢？我无法帮你获得名声、荣耀和追随者，被关注仍然是最困难的部分。但我可以为你指引在线发行的资源，并告诉你如何为不同用途打包你的录音，比如高质量发烧音、低保真宣传MP3、流媒体音频和CD。 



##### 建立在线存在

过去，对于录音艺术家来说，分销和推广是最大的障碍，这也是他们为什么需要与唱片公司签约的原因。现在，由于互联网的便利，分销变得容易了。互联网分销策略有两个部分：你自己的个人网站以及尽可能多的音乐分销和播放网站。始终拥有自己的个人网站，并附上照片、联系信息，以及粉丝可以聆听和购买你音乐的链接。这会提升你的搜索引擎曝光率，也让粉丝更容易找到你。请务必抵制将网站制作成典型的过度制作、充满特效和动画、颜色暗淡而几乎无法阅读的冲动；这样做只会赶走访问者。保持网站简洁、清晰、可读、易于导航，因为没人关心你的网页设计师有多自我放纵。粉丝只想了解你以及如何听到你的音乐，他们不想为此费力。 你应该拥有一致的在线身份。建立在线身份的第一步是注册你自己的域名。不要使用推广别人业务的邮箱地址或域名，例如 carlarockstar@gmail.com 或 guitarcarla@hotmail.com。免费邮箱地址也非常不酷。域名很便宜，每年大约 15 美元。如果你更换网站主机和邮箱服务提供商，你的域名始终可以跟随你。 假设你有一个乐队——比如说你是 The Bandits——你的域名是 http://thebandits.com/。现在，你可以用这个域名做各种创意事情。你乐队的每个成员都可以有自己的邮箱地址，比如 drummer@thebandits.com、diva@thebandits.com 或 rockgod@thebandits.com。无论你尝试什么音乐分销商、托管服务或社交网络网站，http://thebandits.com/ 永远都会在那里，让你的粉丝找到你。 



 有各种各样的域名注册商，例如 Dotster、GoDaddy、Register.com、Tucows 等等。大多数网络托管服务也包括域名注册，但我建议将两者分开。网络托管是一个残酷的行业，充满了来去匆匆的公司，而一个糟糕的注册商可能会使转移到其他注册商变得困难，甚至可能导致你的域名丢失。互联网名称与数字地址分配机构（ICANN）负责监督域名并维护一个认证注册商名单（http://www.icann.org/en/registrars/accredited-list.html）。这并不保证他们的客户服务或价格有多好，只能证明他们是合法的注册商。WebHostingTalk（http://www.webhostingtalk.com/）是获取有关托管服务和注册商的真实信息的绝佳来源。 



 仔细考虑在你的网站上放置哪种联系方式，因为借助互联网的魔力，沟通可能会占用大量时间。如果你不想花很多时间回复电子邮件、推文、论坛帖子等，那么就限制别人联系你的方式。受验证码保护的网页表单是允许粉丝与你交流而不被垃圾信息淹没的好方法。使用简单算术问题或多项选择题（“如果你是人类，请点击小狗的图片。”）的验证码既简单又有效。那种模糊文字的验证码让人烦恼，除了垃圾邮件发送者之外，谁都难以应付。谁是好的网页设计师呢？是能倾听你意见的人；能用简单明了的英语（或你选择的语言）解释他们所做的事的人；并且重视干净、快速、易导航的网站设计，而不是追求炫耀、把粉丝赶走的花哨东西的人。务必在拨号上网和智能手机上测试你的网站。宽带远未普及，而移动互联网使用正在爆炸式增长，所以你需要关注你的网站在慢速连接和小屏幕上的外观和性能。 





> 注意 还要考虑视力障碍者的无障碍性。美国人口老龄化，因此欢迎所有粉丝是明智的商业做法，而无障碍设计对所有访客都是良好设计。如果你的网站设计师不愿意为了让你的网站对所有访客友好而费心，考虑换一个设计师。 





 考虑使用一个租用的传统信箱（邮筒）。它可靠，可以筛掉随意的意识流式 Twitter/电子邮件通信，并确保真正想联系你的人能够联系到你。MySpace 和 Facebook 呢？去试试吧，它们是免费的，而且你可能会惊喜地发现有人在那里找到你。预算一些时间来保持它们的更新。你不必每天登录就能与粉丝进行深入、有意义的交流，但一定要确保你所有的录音和演出信息以及链接都是最新的。建立粉丝群需要时间，所以要有耐心，并仔细阅读所有有助于你最大化利用 MySpace 和 Facebook 的信息。没有快速的神奇解决方案，不要相信任何试图向你兜售这种方案的人。绝不要发送垃圾邮件。不要相信那些声称他们出售真正的、经过保证选择加入的邮件列表，并且里面的人很乐意收到你信息的“营销人员”。这是谎言。你应该仔细建立自己的邮件列表。正确的做法是采用确认订阅方式，这意味着当有人注册时，他们必须回复发送的一封电子邮件以确认他们的注册。 



 让退出比注册更容易；营销人员和垃圾邮件发送者拒绝学习的教训是，你无法强迫别人喜欢你。要小心——网上没有隐私这一说。你说的每句话都是公开表演，并会被永久记录。你必须衡量自己对短信、推特以及其他新兴通信工具的容忍度。它们可能强大有效，也可能让你抓狂。 



##### 在线音乐分销商

与一个独立的在线音乐商店合作，该商店处理销售、给你公平分成，并且不试图侵占你的版权，这不再是幻想，而且你有很多选择。这里是一些网站的示例，它们在分发音乐和补偿艺术家方面有不同的方法。 Magnatune (http://www.magnatune.com/) 的座右铭是“我们不作恶。”他们还有一个我更喜欢的座右铭：“你获得优质音乐，音乐家获得50%。”这是指销售额的50%，而不是利润。Magnatune以多种无DRM格式分发音乐：仅流媒体（就像听收音机）、各种格式的下载、CD，以及商业用途许可。所有音乐都可以先试听后购买，顾客可以选择自己想听的歌曲和专辑，或者收听各种流媒体。Magnatune顾客还可以每月支付15美元购买流媒体会员，享受无限的收听和下载且无广告。Magnatune支持MP3、WAV、FLAC、AAC和Ogg Vorbis格式。WAV和FLAC是最高质量，WAV可以刻录到CD。128Kbps MP3是最低质量，并在创意共享“署名-非商业性使用-相同方式共享”许可下授予。这为非商业用途提供了宽松的权利：共享、混音、采样和翻唱。任何商业用途预期都需支付费用。Magnatune网站上有大量信息，并且可以与真人交流。你保留自己作品的所有版权，并可以与其他分销商达成协议。Magnatune每六个月支付一次艺术家报酬，前提是他们至少赚取了100美元。 CDBaby (http://www.cdbaby.com/) 销售无DRM的CD和MP3下载，并与多个在线商店合作，如iTunes和Amazon。它还提供批量CD复制和封面设计。CDBaby每张专辑收取35美元设置费；这包括数字化CD，将曲目转换为各种格式，扫描封面，以及其他管理任务。你自己定CD价格。CDBaby保留每张唱片4美元和下载销售的25%。艺术家每周支付一次报酬。你的CD将通过许多店铺销售，如Borders和Amazon。 

 CDBaby 为现场演出提供了一项很好的服务——信用卡刷卡机计划。这是一种信用卡/借记卡终端，你可以带到演出现场并放在你的商品桌上使用。CD Baby 会抽取 12.8% 的分成。这听起来很多，但拥有自己的商家信用卡账户成本很高。无论你是否有销售，每月都会收取固定费用，再加上每笔销售的一定比例，通常每笔刷卡交易大约 2.5% 到 3%。如果你在商品桌上销售 CD，CD Baby 抽取 12.8% 的分成，而不是通常的 4 美元手续费。 



 CDBaby 提供许多其他的艺人服务，例如 Host Baby（网站主机服务）、贴纸和卡片，以及预订服务。eMusic (http://www.emusic.com/) 是一家备受好评的无 DRM 在线 MP3 音乐商店，是寻找优秀独立乐队和主流流行音乐的宝库。然而，他们不会接受任何人；他们只和已建立的音乐厂牌合作，因此你需要已经签约某个厂牌。eMusic 完全是企业化、法律化的，充满了官僚作风。对音乐爱好者而言，这可能是个更划算的选择，而对不知名的音乐人来说可能不太适合，但查看一下总不会有坏处。TuneCore (http://www.tunecore.com/) 采用固定费用的系统。你支付每首歌 9.99 美元加 9.99 美元的年度维护费，或者首年支付每张专辑 49.99 美元，其后每年 19.98 美元，再加 19.98 美元的年度维护费。这样你就能进入包括 Amazon MP3、eMusic、Rhapsody 和 Spotify 在内的 19 个在线商店。他们还提供 Amazon 的按需刻录 CD 选项。大多数商店的销售报表有两个月的滞后，而 TuneCore 每月报告两次。在此之后，你可以随时提款（需要 PayPal 账户）。iTunes (http://www.apple.com/itunes/content-providers/faq.html) 是大舞台。这里有很多重要的步骤需要完成，但不经历风雨怎么见彩虹。CD Baby、TuneCore 和 SongCast 都与 iTunes 合作，这比直接申请 iTunes 更方便。独立在线发行联盟（IODA）(http://www.iodalliance.com/) 作为一个集体代理和聚合器，利用集体谈判的力量赋予独立音乐人更强的商业话语权。作为回报，它会收取艺人版权收入的一部分，处理费率谈判、报告、作品分发、版税分配及其他繁琐但必要的商业事务。独立在线发行是一个正在增长的行业，你可以找到更多的发行网站。宣传推广由你自己负责，但每个网站都提供有用的工具和建议来帮助你。不要像典型的无知艺人一样签下不利合同或与一些欺诈型艺人合作。研究发行商很容易，只需坐在电脑前进行网络搜索即可。不要忘记处理复杂的税务问题和细读合同条款。你什么时候能收到报酬？如何核对销售和付款记录？版权、DRM、关闭账户、修改信息、获取真人帮助以及争议解决，都是需要关注的重要事项。考虑按音乐类型选择发行商，而不是按受欢迎程度选择。当然，iTunes 和 Amazon 是大公司，但如果你的市场是高端爵士、舞曲、世界音乐或类似类型，就选择专注于你音乐类型的发行商。 

 保持专业、商务化的态度是好的，并且记住一些平凡的细节，例如遵守承诺；联系信息；正确的拼写、语法和标点；易读的字体；以及美观的图像。没有人关心你在艺术上的痛苦有多深。 



##### 自托管

另一种选择是自托管，这意味着运行你自己的服务器。现代情况与早期的公告板服务（BBS）时代不同，当时成千上万的人在卧室里的电脑上运行自己的BBS服务器。对于动手能力强的人来说，维护一个网站、托管自己的下载资源以及提供作品的流媒体都是可行的，但对于胆小或不擅长技术的人来说则不适合。带宽是最大的问题：如果你想在家里搭建服务器，首先要确认是否能以合理的价格获得足够的带宽，以及你的互联网服务提供商是否允许你运行服务器。随后，你需要负责自己的安全、软件、硬件以及系统和网络管理。成为自己服务器的老板是令人满足的，但确实需要很多技能。更好的选择是使用一个拥有你所需所有工具并提供优惠带宽的优秀托管服务。很难在带宽成本上击败优秀的托管服务，因为他们批量购买并将成本分摊给许多客户。然后，托管服务将处理硬件问题、互联网连接、安全和备用电源。计划范围从低流量网站的免费方案，到价格便宜的共享托管（意味着一个服务器用有限的互联网带宽为许多客户服务），再到付费拥有自己的专用服务器。你可以以低成本开始，并随着成功而升级。 



> 注意 160 记住 WebHosting Talk (http://www.webhostingtalk.com/) 用于了解哪些主机好，哪些应避免。 



 自由/开源软件世界拥有运行自己网站所需的一切。使用 Linux 操作系统，因为它稳健且安全。然后还有很多优秀的服务器可供选择： 



* Drupal、Joomla、Mambo 和 Plone 都是优秀的开源内容管理系统（CMS）的例子。

  这些框架用于管理你网站上的内容，使你能够轻松地组织和发布文章、照片、日历、新闻、论坛和链接。

* Icecast 和 Campcaster 是优秀的开源流媒体 Ogg Vorbis 和 MP3 服务器，你可以将它们嵌入到你的网站上。

* Shoutcast 是一个封闭的专有流媒体服务器，Shoutcast Radio 是一个快速且免费的互联网广播服务，用于托管你的流媒体内容。

  你也可以将 Shoutcast 流嵌入到其他网站中。 



 许多网络托管服务提供 Icecast 和 Shoutcast 流媒体服务，当然，还有许多其他流媒体服务器。任何托管服务都提供一大套服务器软件和一个漂亮的图形控制面板来管理所有内容。如果他们没有某个特定的软件，可以问他们是否会安装它，如果该软件是免费的开源软件，他们很可能会安装。大多数托管服务严重依赖开源软件，因为它免费或价格低廉，而且质量很好。 



 在线销售物品 在线销售可以很简单，比如拥有一个网站和一个PayPal账户来接受付款，或者有一个支票和现金的邮寄地址。支票和汇票欺诈猖獗，可能不值得接受任何一种。PayPal 简单且受欢迎;只要你了解它的收费结构，别对它以各种方式蚕食你的钱感到惊讶。Google Checkout的手续费比PayPal低，也没有PayPal那样不稳定、不公平的争议解决声誉。你可以同时使用两个平台，这样可以吸引更多客户，因为很多人在已有账户后不会愿意注册其中一个。处理在线信用卡支付需要复杂的基础设施结构。你不想成为那种当青少年入侵你的客户数据库，抢走所有信息时登上头条的店铺。考虑使用预制店铺，这些店铺已经具备所有支付流程。从 Amazon.com（http://www.amazonservices.com/ content/sell-on-amazon.htm）开始;它信誉良好且可靠，并且会公布所有价格和政策，所以是做研究和比较的好起点。亚马逊不直接销售独立音乐，但与CD Baby和TuneCore等音乐发行商合作。在亚马逊上开一个店铺可能是展示乐队周边、增加曝光度以及产生CD订单的好渠道。你可以避免支付麻烦，只需在个人网站上托管免费下载和流媒体，然后在CD Baby、Song Cast或你合作的其他发行商上链接你的作品。几乎所有人都忘记的一点是客户服务。客户在有问题或电话时，怎么联系你？邮件？谁会回应？你能指望他们迅速回复，且以让客户满意的方式回应吗？如果你销售的是CD、DVD、T恤和其他品牌商品等实物商品，谁会负责打包和发货，速度多快？客户服务至关重要。无论你的艺术天赋多么出色，获得新客户的成本至少是让现有客户满意的十倍。 
...
数字版权管理 (DRM)，或者有些人更愿意称其为数字限制管理，意味着拷贝保护，以及旨在防止非法复制的各种技术。这个概念听起来可能不错，因为有人免费复制你的音乐听起来像是在占你便宜。但实际上，它是一个噩梦。它甚至无法减缓大规模盗版者，这些人复制并出售成千上万的电影和音乐光盘。它也无法阻止那些想要白拿东西的聪明孩子。它主要妨碍的是合法付费的用户。它给在线分发增加了难以忍受的额外负担，因为这需要始终可用的认证服务器。仅仅几年之后，这一切已经开始崩溃。  沃尔玛在2008年9月宣布将关闭其DRM认证服务器，这意味着如果客户尝试将已购买的DRM“保护”音乐移动到另一台机器或从系统故障中恢复，就无法播放这些音乐。沃尔玛的解决办法是让客户将其DRM“保护”曲目刻录到CD上，这样就可以在任何地方播放。由于公众舆论强烈反对，他们改变了计划，并仍然维护其DRM服务器。尽管他们已经转向无DRM分发，但那个旧的DRM负担将长期困扰他们。苹果的iTunes商店在2008年2月实现了无DRM分发，现有的DRM保护文件可以以每首歌30美分或每张专辑价格的30%的费用转换为无DRM版本。（感谢有机会支付两次费用。）Yahoo! 和Rhapsody也已经放弃了DRM。亚马逊和EMI从一开始就提供无DRM曲目，从而进入并占据了音乐市场的主导份额。 



 有相当多愚蠢的 DRM 方案让我疑惑，为什么有些人能因为想出这些“天才点子”而领取薪水，而我们这些理智的人却必须为温饱而奔波。索尼 BMG 被抓不是一次，而是两次，因为他们偷偷在 Windows 电脑上安装隐藏的 rootkit，通过“保护”的音乐 CD 完成这件事。这不仅是侵犯，如果是某个孩子而不是全球企业，这种行为本应被刑事起诉，而且还在这些电脑上打开了巨大的安全漏洞。其他失败的 DRM 点子也挺让人无语的，比如设置定时炸弹的音乐曲目，在若干天后无法播放，尝试计算并限制可以复制的数量，或者下载的音乐在订阅到期后无法播放。所有这些都依赖某种中心授权和控制，在我看来，这需要对我们的个人事务进行不必要的干涉，并且要求服务提供者维持极高的可靠性，而很少有人能做到。我有比我年纪还大的黑胶唱片——你觉得即便没有 DRM 这些花哨手段，数字曲目能存活多久？我无法理解能想出这些具有敌意、自我挫败的“大哥式”策略的人是怎样的思维方式。另一方面，你的作品被盗取也不好。录音艺术家有权利因其录音作品获得报酬；我并不看好那些说“把你的录音免费送出去，通过现场表演赚钱”的人。显然，他们从未表演过或创作过一张好唱片。我没有什么智慧可分享，唯一能说的是尽量善待你的付费客户。在现实生活中，而不是那些娱乐业大亨所居住的怪异世界里，如果你公平对待别人，并且做正确的事，仅仅因为这是正确的事情而去做，而不是被迫去做，大多数人也会公平对待你。 



##### 版权与法律问题

目前在美国以及其他一些国家，版权法处于一个非常奇怪的状态。版权所有者的权利不断被延长，而合理使用却日益受限，轻微的侵权行为也会被升级为严重犯罪。创作者在自己的作品中被排挤出权利持有者的行列，尽管这是一个长期存在的老问题。正如科特尼·洛芙在《Courtney Love Does the Math》中所说： 



 . . . 一位名叫米奇·格雷泽的国会助理，在美国唱片工业协会（RIAA）的支持下，在一项法案中添加了一项“技术性修正”，将录制音乐定义为1978年《版权法》下的“受雇创作作品”。他是在所有关于该法案的听证会结束后才这样做的。当艺术家们发现这一变化时，已经为时过晚。该法案已送交白宫，等待总统签署。这一版权法的细微更改将在未来几年为唱片公司增加数十亿美元——这些本应支付给艺术家的钱。现如今，“受雇创作作品”将永远归唱片公司所有。根据1978年《版权法》，艺术家可以在35年后重新获得自己作品的版权。如果你创作并录制了《Everybody Hurts》，至少在35年后你可以将其作为家庭遗产收回。但现在，由于这个腐败的小家伙，《Everybody Hurts》永远不会回到你的家人手中，并且可以卖给出价最高的人。 . . . 顺便问一下，你认为唱片行业是利用哪项法案来进行这一修正的？ . . . 1999年的《卫星家庭观看法》怎么样？ 



 那个迷人的小条款在2000年的《委托创作及版权修正法案》中被追溯性废除了。但你也可以看到你将面临的挑战。自己掌控事情的一种方式是以创作共用（Creative Commons）许可发布你的作品。创作共用许可有几种，你可以在 http://www.creativecommons.org/ 查看相关信息。这些许可允许你定义不同程度的重用和分发权限，从宽松到限制。当你寻求法律建议时，一定要找一位精通版权法各方面的律师，包括创作共用相关内容。请注意，我说的是“当你寻求法律建议时”，而不是“如果”。互联网上已经充斥着非律师提供的法律建议，所以请帮自己一个忙，找一个真正的律师为你提供建议。你可以在网上和图书馆找到各种信息，然后在会见律师时，你会有充分的准备。不幸的是，这在现代社会中成为艺术家的一部分是必需的。你可以靠豆子和米饭过活，你可以缝补旧衣服，你可以开着用铁丝和胶带固定的旧车。但解决法律问题是困难的、昂贵的，而且常常是不可能的；预防要便宜得多，也轻松得多。 



##### 了解更多

请参阅第4章以了解有关将现场表演录制到CD上的更多信息。第6章详细介绍了制作高保真音频文件，第9章则完全讲解了在Audacity中进行多轨录音和混音。 
