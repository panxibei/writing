...
安装完毕后，打开服务器上的浏览器，打开 `http://localhost:55414` 即可访问管理配置页面。
...
>
> 文章链接：https://www.sysadm.cc/index.php/xitongyunwei/739-xigmanas-urbackup#comment-708
...
##### 2.1.4.1 在其他 GNU/Linux 发行版上安装服务器
...
##### 2.1.4.2 在其他 GNU/Linux 发行版上以源码方式安装服务器
...
有关GNU/Linux 系统的更多安装提示，请参阅第 `2.1.5` 节，有关独立于操作系统的安装步骤，请参阅第 `2.1.6` 节。



##### 2.1.5 关于 GNU/Linux 服务器安装的小提示
...
* 推荐 `btrfs` ，因为 `UrBackup` 可以将每个文件备份放入单独的子卷中，并且能够在增量文件备份中执行基于块的廉价重复数据删除。
  ...
  有关 如何设置 `btrfs` 备份存储，请参阅第 `11.7.2` 节。

  如果使用 `btrfs` ，你应该设置一个非常低的软文件系统配额（请参阅第 `8.1.14` 节），因为 `btrfs`  目前在空间不足的情况下仍然存在问题，可能需要手动干预。



##### 2.1.6 操作系统独立服务器安装步骤

安装 `UrBackup` 服务器后，您应该执行以下步骤：

* 转到用户设置并添加管理员帐户。如果您不这样做，每个可以访问服务器的人都将能够看到所有备份！

  图14

  

* 通过输入指定的邮件服务器设置来设置邮件服务器（请参阅第 `8.2.1` 节）。

* 如果你希望客户端能够通过 `Internet/广域网` 而不仅仅是通过本地局域网进行备份，请在 `Internet/广域网` 设置中配置服务器的公共服务器名称或 `IP`（参见第 `8.4` 节）。

* 如果你希望**客户端能够通过浏览器访问其备份**并可以通过右键单击指定备份来恢复/访问备份，那么请务必输入服务器的 `URL` 。

  例如你可以这样输入 `http://backups.company.com:55414/` ，确保你的 `DNS` 配置解析正常，如果从内部网络访问，`backups.company.com` 应当指向备份服务器的内部 IP，否则应该解析为外部 IP 地址。

* 你应该在 `UrBackup` 前面放置一个真实的 `Web` 服务器并设置 SSL（请参阅第 `4.2` 节 ）。

* 如果你想查看备份失败的日志，请转到日志界面并为你的电子邮件地址配置报告。

* 根据你的使用场景修改任何其他设置。有关所有可用设置的说明，请参阅第 `8` 节。
