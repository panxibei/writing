...
英文：backup-and-restore-postgresql-with-urbackup

关键字：urbackup,pgsql,postgresql,postgres,database,backup,restore,备份,恢复,数据库,linux,windows,sqldump,wal,percona,sql



在以前哈我估计很多小伙伴可能听都没听说过 `PostgreSQL` （简称 `pgsql` ），而只听说过 `MySQL` 。

毕竟 `MySQL` 流行度不是一般的广，而 `PostgreSQL` 呢被关在了象牙塔里，学术光环反而束缚着她而不为大众所知。

不过这种情况在最近几年也随着 `PostgreSQL` 的排名逐渐靠前而产生了新的变化，越来越多的小伙伴不但认识了 高大上的 `PostgreSQL` ，而且也了解到它在很多方面要比 `MySQL` 更加牛叉。

除去丰富的数据类型定义之外，`PostgreSQL` 相对更加稳定可靠，其实吧，最主要是它不要钱，不要钱啊，这才是要命的重点啊！
